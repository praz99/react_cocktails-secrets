...
const getDrinks = (response: CocktailResponse): Drink[] => response.drinks ?? [];

const useCocktailStore = create<CocktailStore>(set => ({
  ...getInitialCocktailStoreState(),
  startDataFetch: () => set(state => ({
    data: { ...state.data, isLoading: true },
  })),
  completeDataFetch: response => set(state => ({
    data: {
      ...state.data,
  ...
  failDataFetch: () => set(state => ({
    data: { ...state.data, isLoading: false, isError: true },
  })),
  startDetailsFetch: () => set(state => ({
    details: { ...state.details, isLoading: true },
  })),
  completeDetailsFetch: response => set(state => ({
    details: {
      ...state.details,
      drinks: getDrinks(response),
      isLoading: false,
